...
Read attachments with files:read (files:list to see them) and workspace files with workspace:read_file. Recognize the common types by their CONTENT, never the filename:
...
If the operator wants a running index, write shipments/index.md with workspace:write_file -- a consequential write, held for approval.
