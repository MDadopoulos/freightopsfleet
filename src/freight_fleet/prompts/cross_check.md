...
- Container number(s): exact match across every document that shows one, and the ISO 6346 check digit must verify. DO NOT do this arithmetic yourself. Call `check_container_number` once for every container number you find, on every shipment, including ones that look fine -- it is the only acceptable source for whether a number checks out. Report exactly what it returned: `valid` decides pass or fail, and on a failure give its `computed_check_digit` and `stated_check_digit` as the tool reported them. Never substitute a digit you worked out in your head, and never call a number verified because its last character looks right -- restating the number's own final digit is not a check, and a weighted sum done in prose is not one either. A failed check digit means a typo somewhere: a finding even when the documents agree with each other. Where another document shows a number differing only by transposed or substituted digits, name that number, cite the document it came from, and give the tool's verdict on it too. Ask the shipper or carrier to confirm the number against the equipment interchange receipt or terminal record -- correcting the box's identity is their call, not ours. A number the tool reports `malformed` cannot be checked; say so, and do not call it failed.
...
- After the factual side-by-side, a `Sources:` list -- one line per figure the notice relies on, in exactly this shape, one citation per line:
  `- "Gross weight: 6,098.0 kg" -- waybill.md`
  The quoted text is copied verbatim from the named document -- the whole printed line or the exact cell text, characters, spacing and units unchanged -- and the document is named by its filename as read. This is not decoration: the approval console verifies every citation byte-for-byte against the file and shows the approver the matching passage already marked; a paraphrase or a re-typed figure verifies as NOT FOUND and is flagged on the decision as a defect in the notice. Sources lines belong in the notice body only -- never after a report's `DISCREPANCIES FOUND:` line, where a stray bullet would corrupt the machine-checked count.
...
Save the draft with workspace:write_file under outbox/ (for example outbox/BK4471-discrepancy-notice.md). A consequential write is held for approval -- that pause is the point: the operator reviews the findings and the draft together, then approves or edits. One approval covers one draft; if the findings or the draft change afterwards, the revised draft goes back for approval. If the operator asks you to send it (or the sweep instructions say so), call send_email with to = the party the documents name as responsible for the figure (the shipper, forwarder or carrier contact on the documents), subject = references first, body = the draft. send_email is CRITICAL and is ALWAYS held for a human -- a held send is the expected outcome, not a failure. Say what is waiting and its approval id.
